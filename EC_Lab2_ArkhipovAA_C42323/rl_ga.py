...
class RL_ga_experiment:
    def factory(self):
        ...
        for i in range(len(self.params)):
            if i % 2 == 0:
                individual.append(np.random.normal(0.1, 0.3,
                                                   size=self.params[i].shape))
            else:
    ...
    def run(self):
        ...
        pop, log = ga_scheme.eaMuPlusLambda(pop, self.engine, mu=self.pop_size,
                                            lambda_=int(0.8 * self.pop_size),
                                            cxpb=0.4, mutpb=0.4,
                                            ngen=self.iterations, verbose=True,
    ...
    def fitness(self, individual):
        ...
        for _ in range(1):
            ...
            for t in range(200):
                self.env.render()
                act_prob = \
                    self.model.predict(state.reshape(1, self.input_dim)).\
                        squeeze()
                action = np.random.choice(np.arange(self.output_dim), 1,
                                          p=act_prob)[0]
