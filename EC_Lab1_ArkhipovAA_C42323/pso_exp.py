...
creator.create("Particle", np.ndarray, fitness=creator.BaseFitness,
               speed=None, smin=None, smax=None, best=None)


class PSOAlg:
    def generate(self):
        return np.random.uniform(self.pmin, self.pmax, self.dimension)
    ...
    def updateParticle(self, part, best):
        v1 = (part.best - part) * np.random.uniform(0, self.c1)
        v2 = (best - part) * np.random.uniform(0, self.c2)
        part.speed = np.clip(part.speed * self.w + v1 + v2, self.smin,
                             self.smax)
