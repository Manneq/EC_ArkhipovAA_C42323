...
def read_log(log):
    ...
    return np.array(gen_list), np.array(avg_list), np.array(std_list), \
        np.array(max_list), np.array(min_list)


def draw_log(log, title,
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("plots/" + title + ".png", dpi=dpi)
    plt.show()
...
def draw_logs(log1, log2, lab1, lab2):
    ...
    plt.plot(gen1, max1, label="{}_max".format(lab1), color="blue",
             linewidth=2)
    plt.fill_between(gen1, avg1 - std1, avg1 + std1, alpha=0.2, color="blue")
    plt.plot(gen2, avg2, label=lab2, color="orange")
    plt.plot(gen2, max2, label="{}_max".format(lab2), color="orange",
             linewidth=2)
